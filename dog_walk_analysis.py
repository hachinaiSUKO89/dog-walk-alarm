...
路面温度の基準カーブ(直射日光下・晴天時を想定した高温側の値)は、以下の
実測データ・研究に基づいて較正している(厳密な物理計算ではなく概算)。

- 石黒覚・山中正善「充填モルタルによるアスファルト舗装路面の温度制御」
  コンクリート工学年次論文集 Vol.34 No.1, 2012.
  https://data.jci-net.or.jp/data_pdf/34/034-01-1223.pdf
  → 8月の晴天日、密粒度アスファルトの路面最高温度は連日60℃を超えた。
- 丸山ひかる他「夏季のアスファルト舗装工事における保存雪散布等による
  交通開放時間短縮効果の検討」NEXCO新潟/長岡技術科学大学, 2024実測.
  https://www.thr.mlit.go.jp/yukimirai_odate/ronbun/2_05.pdf
  → 平均外気温33.3℃・晴れ・15〜16時の実測で、既設アスファルト路面温度は
    約55〜65℃(既設路面は40℃超)。
- 神奈川県自然環境保全センター研究報告「アスファルト及び芝生上における
  暑熱環境の比較調査について」.
  https://www.pref.kanagawa.jp/documents/3463/kenshi2019-3.pdf
  → 炎天下でアスファルト59.8℃に対し芝生は40.6℃(約19℃低い)。芝生面は
    蒸発散により昇温しにくく、18時以降は気温より低くなることもある。
- 個人による実測ブログ(2021年7月, 気温32.6℃・晴れ):
  https://kanmegu.com/asphalt-temperature-summer-20210731/
  → アスファルト56.8℃、土(地面)37.8℃。

これらの実測値はいずれも「気温30℃前後・晴天・日中」の条件のため、
低温域(20℃以下)や夜間・悪天候時の値は直接の実測データがなく、上記の
傾向から外挿した概算値である点に留意すること。そのため、実際の天気
...
# (気温, 推定路面温度) の基準点。直射日光下・晴天時を想定した高めの値。
# 25〜35℃付近は上記の実測データに合わせて較正済み。土・芝は蒸発散により
# 保熱しにくいため上昇幅を大きく小さく設定している(芝生の実測知見を反映)。
_ASPHALT_POINTS = [
    (0, 8), (10, 18), (15, 25), (20, 33), (25, 48), (30, 58), (33, 60), (35, 63), (40, 68),
]
_SOIL_POINTS = [
    (0, 3), (10, 13), (15, 19), (20, 25), (25, 31), (30, 37), (33, 40), (35, 42), (40, 46),
]
